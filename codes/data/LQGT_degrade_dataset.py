...
class LQGTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.paths_LQ:
            ...
            #     img_LQ = cv2.GaussianBlur(img_LQ.copy(), (15, 15), sigma)
            
            # using matlab imresize
            if scale == 1:
                img_LQ = img_LQ
            else:
                img_LQ = util.imresize_np(img_LQ, 1 / scale, True)
            
                
            # add noise
            if random.uniform(0, 1) > 0.25:
                noise_level = np.random.uniform(0, 30) / 255.0
                noise_map = np.random.normal(loc=0.0, scale=1.0, size=img_LQ.shape)*noise_level
                img_LQ += noise_map
