...
def degradation_bsrgan_plus_lyh(img, blur_prob, gaussian_prob, poisson_prob, speckle_prob, jpeg_prob, sf=1, shuffle_prob=0.5, use_sharp=False):
    h1, w1 = img.shape[:2]
    img = img.copy()
    h, w = img.shape[:2]
    ...
    for i in shuffle_order:
        if i == 0:
            ...
            if random.random() < gaussian_prob:
                img = add_Gaussian_noise(img, noise_level1=2, noise_level2=30)
    ...
    return img, hq, degradation_record

if __name__ == '__main__':
    img = util.imread_uint('/data1/yhliu/DATA/ClassicalSR/urban100/img_001.png', 3)
    img = util.uint2single(img)
    sf = 1
    
    for i in range(5):
        img_lq, img_hq, degradation_record = degradation_bsrgan_plus_lyh(img)
        print(i)
