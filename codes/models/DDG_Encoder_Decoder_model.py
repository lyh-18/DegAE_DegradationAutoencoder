...
class DDGModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            pred_g_fake = self.netD_DDG(self.fake_H)
            if self.opt['train']['gan_type'] in ['gan', 'lsgan']:
                l_g_gan = self.l_gan_w * self.cri_gan(pred_g_fake, True)
            elif self.opt['train']['gan_type'] == 'ragan':
                pred_d_real = self.netD_DDG(self.var_ref).detach()
        ...
        l_d_total = 0
        pred_d_real = self.netD_DDG(self.var_ref)
        pred_d_fake = self.netD_DDG(self.fake_H.detach())  # detach to avoid BP to G
        if self.opt['train']['gan_type'] in ['gan', 'lsgan']:
