...
class DDG_SRGAN_Model(BaseModel):
    def __init__(self, opt):
        super(DDG_SRGAN_Model, self).__init__(opt)
        if opt['dist']:
        ...
        if train_opt['ft_from'] is not None:
            self.netEncoder = networks.define_DDE(opt).to(self.device)
        self.netG = networks.define_DDG(opt).to(self.device)
        ...
        if self.is_train:
            self.netD = networks.define_D_DDG(opt).to(self.device)
            if opt['dist']:
        ...
        if self.is_train:
            ...
            ft_from = train_opt['ft_from']
            optim_params = []
            ft_flag = False
            for k, v in self.netG.named_parameters():  # can optimize for a part of the model
                #print(k)
                if (ft_flag == False and k.split('.')[1]+k.split('.')[2] == ft_from) or ft_from == None:
                    ft_flag = True

                if v.requires_grad and ft_flag:
    ...
    def optimize_parameters(self, step):
        if self.opt['train']['ft_from'] is not None:
            for p in self.netEncoder.parameters():
        ...
        if self.opt['network_DDG']['require_modulation']:
            self.embedding1_ref = self.netEncoder(xxx)
            self.fake_H = self.netG(self.var_L, self.embedding1_ref)
        else:
            self.fake_H = self.netG(self.var_L, self.var_L)
