...
class Uformer_Encoder(nn.Module):
...
class Uformer_Decoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        conv0, conv1, conv2, conv3, conv4 = x[0], x[1], x[2], x[3], x[4]
        #Decoder
...
class Uformer_Decoder_modulation(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        self.decoderlayer_0 = BasicUformerLayer(dim=embed_dim*16,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)
        self.modulation_layer0 = GFM_modulation(degradation_embed_dim, embed_dim*16, noise=True)
        ...
        self.decoderlayer_1 = BasicUformerLayer(dim=embed_dim*8,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)
        self.modulation_layer1 = GFM_modulation(degradation_embed_dim, embed_dim*8, noise=True)
        ...
        self.decoderlayer_2 = BasicUformerLayer(dim=embed_dim*4,
                            ...
                            token_projection=token_projection,token_mlp=token_mlp,shift_flag=shift_flag,
                            modulator=modulator,cross_modulator=cross_modulator)
        self.modulation_layer2 = GFM_modulation(degradation_embed_dim, embed_dim*4, noise=True)
    ...
    def forward(self, x, embedding=None, mask=None):
        ...
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0,mask=mask)
        deconv0 = self.modulation_layer0(deconv0, embedding)
        
        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1,mask=mask)
        deconv1 = self.modulation_layer1(deconv1, embedding)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2,mask=mask)
        deconv2 = self.modulation_layer2(deconv2, embedding)
...
if __name__ == "__main__":
    input_size = 256
    encoder_depths=[1, 2, 8, 8, 2]
    decoder_depths=[8, 8, 2, 1]
    depths=[1, 2, 8, 8, 2, 8, 8, 2, 1]
    model_encoder = Uformer_Encoder(img_size=input_size, embed_dim=16,depths=depths,
                 win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    model_decoder = Uformer_Decoder(img_size=input_size, embed_dim=16,depths=depths,
                 win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    model_decoder_modulation = Uformer_Decoder_modulation(img_size=input_size, embed_dim=16,depths=depths,
                 win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)

    x = torch.randn((1, 3, 480, 500))
    B, C, H, W = x.shape
    print(x.shape)
    x, mask = expand2square(x, factor=128)
    print(x.shape)
    inter_fea = model_encoder(x)
    y = model_decoder(inter_fea)
    print(y.shape)
    y = torch.masked_select(y, mask.bool()).reshape(1,3,H,W)
    print(y.shape)
    print('#############################')
    
    embed = torch.randn((1, 512, 1, 1))
    x = torch.randn((1, 3, 480, 500))
    B, C, H, W = x.shape
    print(x.shape)
    x, mask = expand2square(x, factor=128)
    print(x.shape)
    inter_fea = model_encoder(x)
    y = model_decoder_modulation(inter_fea, embed)
    print(y.shape)
    y = torch.masked_select(y, mask.bool()).reshape(1,3,H,W)
    print(y.shape)
